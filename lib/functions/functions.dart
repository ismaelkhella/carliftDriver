...
import 'package:tagxisuperdriver/pages/onTripPage/digitalsignature.dart';
import 'package:tagxisuperdriver/pages/onTripPage/droplocation.dart';
import 'package:tagxisuperdriver/pages/onTripPage/invoice.dart';
...
import 'package:tagxisuperdriver/pages/vehicleInformations/vehicle_color.dart';
import 'package:tagxisuperdriver/pages/vehicleInformations/vehicle_make.dart';
import 'package:tagxisuperdriver/pages/vehicleInformations/vehicle_model.dart';
import 'package:tagxisuperdriver/pages/vehicleInformations/vehicle_number.dart';
import 'package:tagxisuperdriver/pages/vehicleInformations/vehicle_type.dart';
import 'package:http/http.dart' as http;
import 'package:firebase_auth/firebase_auth.dart';
import 'package:firebase_database/firebase_database.dart';
import 'package:firebase_messaging/firebase_messaging.dart';
import 'package:tagxisuperdriver/pages/vehicleInformations/vehicle_year.dart';
...
String audio = 'audio/driverneworder.mp3';
String audio1 = 'audio/newnotification.mp3';
String audioNewMessege = 'audio/newmessagefromclient.mp3';
String audioEndTripSound = 'audio/endTrip.mpeg';
String audioClientCancel = 'audio/clientCancel.mpeg';
String audioAcceptNewOrder = 'audio/afteracceptdriver.mpeg';
String audioStartTrip = 'audio/startTrip.mpeg';
String audioIsArrived = 'audio/isArraived.mpeg';
bool internet = true;
dynamic centerCheck;
...
//base url
String url =
    'http://carlift.taxi/'; //add '/' at the end of the url as 'https:website.com/'
String mapkey = 'AIzaSyD-jxJWVCPF4ASZS_yvKWTshTuRfUMcpck';
String mapStyle = '';
...
getvehicleType() async {
  ...
  try {
    ...
    print("$myServiceId $transportType");
    if (response.statusCode == 200) {
...
takeCommissionFromDriver(String totalAmount, String adminCommission, String type) async {
  dynamic result;
  try {
    var response = await http

        .post(Uri.parse('${url}api/v1/request/take-commission-from-driver'), headers: {
...
userTotalPayCash(String totalAmount, String requestId, String amount) async {
  dynamic result;
  try {
    var response = await http
        .post(Uri.parse('${url}api/v1/request/end/cash'), headers: {
      // 'Content-Type' : 'application/json',
      'Authorization': 'Bearer ${bearerToken[0].token}',
    }, body: {
      'totalAmount': totalAmount,
...
registerOwner() async {
  ...
  try {
    ...
    print("mobile: $phnumber");
    print("email: $email");
    print("address: $companyAddress");
    print("postal_code: $postalCode");
    print("city: $city");
    print("tax_number: $taxNumber");
    print("company_name:$companyName");
    print("device_token: $fcm");
    print("lang: $choosenLanguage");
    print("transport_type: $transportType");
    print("email_confirmed: ${(value == 0) ? '0' : '1'}");
    print("country: ${(countries[phcode]['dial_code'])}");
    print("service_location_id: $ownerServiceLocation");
    print("login_by: ${((platform == TargetPlatform.android) ? 'android' : 'ios')}");
      ...
      if (userDetails['role'] != 'owner') {
        ...
        if (userDetails['onTripRequest'] != null) {
          ...
          if (driverReq['is_driver_arrived'] == 1 &&
            ...
            audioPlayers.play(AssetSource(audioIsArrived));
          ...
          if (driverReq['is_completed'] == 0 &&
              ...
              rideTimer == null &&
              driverReq['is_rental'] != true) {
            audioPlayers.play(AssetSource(audioStartTrip));
          ...
          //   audioPlayers.play(AssetSource(audio1));
          // }
          chatList.clear();
          driverReq = {};
          valueNotifierHome.incrementNotifier();
...
audioPlay() async {
  audioPlayers.play(AssetSource('audio/neworder.mp3'));
...
etaRequest() async {
  dynamic result;
  try {
    print(addressList.firstWhere((e) => e.id == 'pickup').latlng.latitude);
    print(
...
endTrip() async {
  ...
  try {
    ...
    print(driverReq['requestBill']);
    print('-==============================');

    print(driverReq['request_number']);
    print(driverReq['service_location_id']);
    print(driverReq['total_distance']);
    print(driverReq['is_completed']);
    print(driverReq['pick_lat']);
    print(driverReq['pick_lng']);
    print(driverReq['drop_lat']);
    print(driverReq['drop_lng']);
    print(driverReq['pick_address']);
    print(driverReq['drop_address']);

    // print("$dist, ${center.latitude},  ${center.longitude}");
    if (response.statusCode == 200) {
      audioPlayers.play(AssetSource(audioEndTripSound));
      ...
      if (jsonDecode(response.body)['success'] == true) {
        if (chatList.where((element) => element['from_type'] == 1).length !=
            jsonDecode(response.body)['data']
                ...
                .length) {
          audioPlayers.play(AssetSource(audioNewMessege));
        }
...
geolocs.LocationSettings locationSettings = (platform == TargetPlatform.android)
    ? geolocs.AndroidSettings(
        ...
        foregroundNotificationConfig:
          ...
          notificationText:
              "Carlift Driver will continue to receive your location in background",
